...
#define PRE_START_STATS()  \
    ...
    unsigned long _ldextcycles = 0; \
    unsigned long _stext = 0; \
    unsigned long _stextcycles = 0; \
    ...
    for(int _k=0; _k<HOTTING+REPEAT; _k++) { \
      pi_perf_conf((1<<PI_PERF_CYCLES) | (1<<PI_PERF_INSTR) | (1<<PI_PERF_ACTIVE_CYCLES) | (1<<PI_PERF_LD_EXT) | (1<<PI_PERF_LD_EXT_CYC)| (1<<PI_PERF_ST_EXT) | (1<<PI_PERF_ST_EXT_CYC)| (1<<PI_PERF_TCDM_CONT) | (1<<PI_PERF_LD_STALL) | (1<<PI_PERF_IMISS) | (1<<0x11)| (1<<0x12)| (1<<0x13)| (1<<0x14));
...
#define STOP_STATS() \
   ...
   if (_k >= HOTTING) \
    { \
      ...
      _ldextcycles    += pi_perf_read (PI_PERF_LD_EXT_CYC); \
      _stext    += pi_perf_read (PI_PERF_ST_EXT); \
      _stextcycles    += pi_perf_read (PI_PERF_ST_EXT_CYC); \
   ...
   if (_k == HOTTING+REPEAT-1) \
    { \
      ...
      printf("[%d] ext load cycles= %lu\n", id, _ldextcycles/REPEAT); \
      printf("[%d] ext store = %lu\n", id, _stext/REPEAT); \
      printf("[%d] ext store cycles= %lu\n", id, _stextcycles/REPEAT); \
